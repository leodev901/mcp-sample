...
## 🚀 Grafana Cloud (SaaS) 무료 연동 튜토리얼 (A to Z)

서버(k9s 등)에 직접 무거운 인프라를 올리지 않고, 클라우드 서비스를 이용해 10분 만에 이 프로젝트의 로그를 눈으로 확인하는 방법입니다.

### Step 1. Grafana Cloud 계정 생성 (무료)
1. **[Grafana Cloud 웹사이트(grafana.com)](https://grafana.com/products/cloud/)** 에 접속합니다.
2. `Create free account` (또는 `Get Started for Free`) 버튼을 클릭합니다.
3. 이메일(Google/GitHub 연동 지원)로 가입을 완료하고, 나만의 클라우드 전용 주소(ex: `myproject.grafana.net`)를 할당받습니다.

### Step 2. OpenTelemetry 연결 정보(Endpoint & Token) 발급받기
1. Grafana Cloud 대시보드 메인 화면 좌측 메뉴에서 **`Connections` -> `Add new connection`**을 클릭합니다.
2. 검색창에 **`OpenTelemetry`**를 검색해서 클릭합니다.
3. `Configure OpenTelemetry` 화면에서 **`Generate Token`**(토큰 생성) 버튼을 누릅니다.
4. 화면에 3가지 중요한 정보가 나타납니다. 이를 반드시 메모장에 복사해 둡니다.
   - `OTLP Endpoint` (예: `https://otlp-gateway-prod-us-east-0.grafana.net/otlp`)
   - `Instance ID` (또는 Username)
   - `API Token` (또는 Password)

### Step 3. 내 프로젝트 코드(`logger.py` 및 `.env`) 수정하기
Grafana 클라우드는 보안을 위해 `HTTPS` 통신과 `인증 헤더(Header)`를 요구합니다. 이 프로젝트에 맞게 수정을 진행해 주세요.

**1) `.env` 파일에 발급받은 정보 추가**
```env
# .env 파일
GRAFANA_ENDPOINT=https://otlp-gateway-prod-us-east-0.grafana.net/otlp/v1/logs
GRAFANA_INSTANCE_ID=123456
GRAFANA_API_TOKEN=glc_...
```
*(참고: 파이썬 OTLP에서는 엔드포인트 주소 끝에 `/v1/logs`를 붙여주어야 정확히 작동하는 경우가 많습니다.)*

**2) `app/common/logger.py`의 Exporter 코드 수정**
기존에는 내부망(HTTP)용으로 뚫려있던 `insecure=True` 코드를 지우고, 클라우드 인증키를 심어줍니다.
```python
import base64
import os
...
# (중략...)
        ...
        password = os.getenv("GRAFANA_API_TOKEN", "")
        auth_string = base64.b64encode(f"{username}:{password}".encode()).decode()

        # 보안 연결(HTTPS) 사용 및 헤더에 인증키 추가
        exporter = OTLPLogExporter(
            endpoint=os.getenv("GRAFANA_ENDPOINT"),
            insecure=False,  # HTTP 대신 HTTPS 사용
            headers=(("authorization", f"Basic {auth_string}"),)
        )
```

### Step 4. 앱 실행 & Grafana에서 로그 확인!
1. FastMCP 서버를 실행(`uvicorn app.main:app...`)하고 몇 가지 도구(달력 조회 등)를 호출하여 로그를 발생시킵니다.
2. 1~2분 뒤 다시 Grafana Cloud 웹사이트로 돌아갑니다.
3. 좌측 메뉴에서 **`Explore`** (나침반 모양 아이콘)를 클릭합니다.
4. 상단 데이터 소스(Data Source) 선택 창에서 **`grafanacloud-이름-logs (Loki)`**를 선택합니다.
5. 아래쪽 `Label filters`에서 `service_name` `=` `mcp-ms365` 형태로 검색하면 방금 내 로컬 컴퓨터에서 발생한 로그들이 예쁘게 찍혀있는 것을 실시간으로 확인할 수 있습니다! 🎉
